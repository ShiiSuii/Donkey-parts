...
def safe_imshow(winname, img):
    try:
        ...
        pass  # Evita errores si no hay entorno gráfico
...
class FIRAEngine(object):
    def __init__(self, tag_dict, proximity_thresholds, apriltag_hz, zebra_hz, top_crop_ratio,
                 ...
                 proceed_correction_duration=1.0, proceed_straight_duration=1.0,
    ...
    def get_realsense_frame(self):
        ...
        if not color_frame:
            return None
        return np.ascontiguousarray(np.asarray(color_frame.get_data()))
    ...
    def detect_lane_and_correction(self, img):
        ...
        if lines is not None:
            ...
            if self.debug_visuals:
                ...
                cv2.arrowedLine(img, (img.shape[1] // 2, img.shape[0]),
                                (int((x1 + x2) / 2), int((y1 + y2) / 2)), (255, 0, 0), 2)
