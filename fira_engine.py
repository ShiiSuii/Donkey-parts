...
def safe_imshow(winname, img):
    try:
        ...
        pass  # Evita el crash si no hay entorno gráfico
...
class FIRAEngine(object):
    def __init__(self, tag_dict, proximity_thresholds, apriltag_hz, zebra_hz, top_crop_ratio,
                 stop_duration=5, turn_duration=2, wait_duration=3.0, turn_initial_wait_duration=1.0,
                 proceed_correction_duration=1.5, proceed_straight_duration=1.5,
                 debug_visuals=True, debug=False):

        import os
        if debug_visuals and not os.environ.get("DISPLAY"):
            print("⚠️ DEBUG_VISUALS desactivado: no hay entorno gráfico (DISPLAY no definido).")
            debug_visuals = False
    ...
    def crop_image(self, img, crop_ratio=None):
        ...
        return img[int(img_height * crop_ratio):, :]
    ...
    def detect_lane_and_correction(self, img):
        ...
        if lines is not None:
            ...
            if self.debug_visuals:
                ...
                cv2.arrowedLine(img, (int(img_center_x), img.shape[0]), (int((x1 + x2) / 2), int((y1 + y2) / 2)), (255, 0, 0), 2)
    ...
    def run(self, angle, throttle, input_img_arr):
        ...
        if self.state == 'wait-for-crosswalk':
            ...
            if len(lines) >= 5:
                ...
                self.stop_start_time = current_time
                if self.debug_visuals:
                    self.zebra_crosswalk_detector.draw_crosswalk_lines(lines, cropped_realsense_img)
                return 0, 0, input_img_arr
            return angle, throttle, input_img_arr
        ...
        if self.state in ['turn_left', 'turn_right']:
            ...
            if self.turn_manager.is_turning():
                return (-1 if self.state == 'turn_left' else 1), 1, input_img_arr
        ...
        if self.state == 'correction_after_turn':
            if self.debug_visuals:
                safe_imshow('Street', show_img)
            if self.proceed_manager.is_correcting():
                correction_angle, show_img = self.detect_lane_and_correction(show_img)
        ...
        if self.state == 'idle':
            if current_time - self.last_apriltag_detection_time >= 1.0 / self.apriltag_hz:
                ...
                angle, throttle, cropped_realsense_img = self.detect_apriltags_and_update_state(
                    cropped_realsense_img, current_time, throttle, angle)

        if self.debug_visuals:
            safe_imshow('Realsense', cropped_realsense_img)
            safe_imshow('Street', show_img)
